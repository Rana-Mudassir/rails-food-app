...
RSpec.describe "RecipeFoods", type: :request do
  before do
    ...
    sign_in @user
    @recipe = Recipe.create(user: @user, name: 'Test Recipe', preparation_time: 10, cooking_time: 20, description: 'A delicious recipe', public: true)
  ...
  describe 'POST /recipe_foods' do
    ...
    let!(:recipe) { Recipe.create(user: @user, name: 'Test Recipe', preparation_time: 10, cooking_time: 20, description: 'A delicious recipe', public: true) }
  ...
  describe 'DELETE /recipe_foods/:id' do
    let!(:food) { Food.create(name: 'Ingredient 1') }
    let!(:recipe) { Recipe.create(user: @user, name: 'Test Recipe', preparation_time: 10, cooking_time: 20, description: 'A delicious recipe', public: true) }
    let!(:recipe_food) { RecipeFood.create(recipe: @recipe, food: @food, quantity: 2) } # Use @recipe instead of recipe and @food instead of food
